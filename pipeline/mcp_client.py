...
from retry import MAX_ATTEMPTS, MEMCLAW_BASE_SECONDS, backoff_delay
    ...
    for attempt in range(MAX_ATTEMPTS):
        try:
            ...
            if attempt < MAX_ATTEMPTS - 1:
                time.sleep(wait)
                continue
            raise RequestException(f"MemClaw MCP {method} failed after {MAX_ATTEMPTS} attempts: {exc}") from exc
